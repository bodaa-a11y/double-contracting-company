...
import { X, Calendar, UserCheck, ChevronRight, ChevronLeft, ShieldCheck, Image as ImageIcon } from 'lucide-react';
    ...
    <AnimatePresence>
      <div className="fixed inset-0 z-50 flex items-center justify-center p-3 sm:p-6 overflow-y-auto" data-lenis-prevent>
        ...
        <motion.div
          ...
          className="relative w-full max-w-4xl max-h-[92vh] flex flex-col bg-[#39424C] border border-[#C9A227]/40 rounded-3xl shadow-2xl text-white overflow-hidden z-10 my-4"
        >
          {/* Close Button */}
          <button
            onClick={onClose}
            className="absolute top-4 left-4 z-30 p-2.5 rounded-full bg-black/70 hover:bg-black/90 text-white transition-all cursor-pointer border border-white/20 shadow-lg"
            aria-label="إغلاق"
          >
            <X className="w-5 h-5" />
          </button>
          ...
          {/* Top Category Tag */}
          <div className="absolute top-4 right-4 z-30 bg-[#116B43] text-white px-4 py-1.5 rounded-full text-xs font-bold border border-[#C9A227]/50 shadow-lg flex items-center gap-1.5">
          ...
          <div className="relative w-full h-[250px] sm:h-[420px] bg-black/95 flex items-center justify-center overflow-hidden flex-shrink-0">
            ...
            {/* Photo Counter Badge */}
            {images.length > 1 && (
              <div className="absolute bottom-3 left-4 bg-black/75 backdrop-blur-md px-3 py-1 rounded-full text-xs font-bold text-gray-200 border border-white/20 flex items-center gap-1.5 z-20">
                <ImageIcon className="w-3.5 h-3.5 text-[#C9A227]" />
                <span dir="ltr">{activeImageIndex + 1} / {images.length}</span>
              </div>
            )}
            ...
            {images.length > 1 && (
              <>
                <button
                  onClick={prevImage}
                  className="absolute right-3 top-1/2 -translate-y-1/2 p-3 rounded-full bg-black/60 hover:bg-[#116B43] text-white transition-all cursor-pointer border border-white/20 shadow-xl z-20"
                ...
                <button
                  onClick={nextImage}
                  className="absolute left-3 top-1/2 -translate-y-1/2 p-3 rounded-full bg-black/60 hover:bg-[#116B43] text-white transition-all cursor-pointer border border-white/20 shadow-xl z-20"
          ...
          {/* Dedicated Separate Thumbnails Strip */}
          {images.length > 1 && (
            <div className="w-full bg-[#242A31] p-3 border-t border-b border-white/10 flex items-center gap-2.5 overflow-x-auto scrollbar-thin scrollbar-thumb-[#C9A227]/40 flex-shrink-0">
              {images.map((img, idx) => (
                <button
                  key={idx}
                  ...
                  className={`relative flex-shrink-0 w-16 h-12 sm:w-20 sm:h-14 rounded-xl overflow-hidden border-2 transition-all cursor-pointer ${
                    activeImageIndex === idx
                      ? 'border-[#C9A227] scale-105 shadow-md ring-2 ring-[#C9A227]/30'
          ...
          <div className="p-6 sm:p-8 space-y-6 text-right overflow-y-auto flex-1">
            <div>
